...
MUTATIONS = [
    ...
    (ADDON,
     ...
     '"tailscale": _("LAN: low latency"),',
     "py", "two settings sharing one spoken label"),
    (ADDON,
     '"wired": {"prebufferMs": 5, "outputLatencyMs": 5, "bufferMs": 60, "opusFrameMs": 5},',
     '"wired": {"prebufferMs": 15, "outputLatencyMs": 15, "bufferMs": 120, "opusFrameMs": 5},',
     "py", "a wired profile loosened until it saves nothing over LAN"),
    (ADDON,
    ...
    (ADDON,
     ...
     '\t\tif False:\n\t\t\tself._speak(_("Remote audio helper is missing"))\n\t\t\treturn',
     "py", "a missing helper binary going unreported"),
    (ADDON,
    ...
    # --- The RemSound-compatible transport ----------------------------------
    (ADDON,
     '\t\tif not entry or len(entry) > 255 or any(c not in _PEER_ALLOWED for c in entry):\n\t\t\tcontinue',
     '\t\tif not entry:\n\t\t\tcontinue',
     "py", "a malformed RemSound address reaching the helper, which refuses to start"),
    (ADDON,
     '\t\tif not str(config.get("password") or ""):',
     '\t\tif False:',
     "py", "a RemSound connection starting with no password, which can never connect"),
    (ADDON,
     '\tif role == "duplex":',
     '\tif False:',
     "py", "duplex over the one-way relay server being allowed"),
    (ADDON,
     '\t\tif isinstance(volume, int) and self._volumeCallback is not None:',
     '\t\tif False:',
     "py", "a live volume change not remembered for the next connection"),
    (ADDON,
     'process.stdin.write("!" + command + "\\n")',
     'process.stdin.write(command + "\\n")',
     "py", "a live command sent without its marker, stopping the helper instead"),
    (ADDON,
     '\tif config.get("transport") == "remsound" and not str(config.get("password") or ""):',
     '\tif config.get("transport") == "remsound":',
     "py", "a password reminder shown for a RemSound setup that already has one"),
    (ADDON,
     '\tif running:\n\t\treturn ""',
     '\tif False:\n\t\treturn ""',
     "py", "a connection that is already running still told how to start"),
    (ADDON,
     '"transport": "remsound",',
     '"transport": "nvda",',
     "py", "a new install not getting the connection type this add-on leads with"),
    (ADDON,
     '\t\t\tif loaded and "transport" not in loaded:\n\t\t\t\tloaded = dict(loaded, transport="nvda")',
     '\t\t\tif False:\n\t\t\t\tloaded = dict(loaded, transport="nvda")',
     "py", "an existing relay setup silently moved onto RemSound by an upgrade"),
    ("helper/RemSoundProtocol.cs",
     "Pbkdf2Iterations = 100_000",
     "Pbkdf2Iterations = 50_000",
     "cs", "the RemSound key derivation drifting away from every other app"),
    ("helper/RemSoundProtocol.cs",
     "for (var i = 0; i < RemCrypto.NonceBytes - PrefixBytes; i++)",
     "for (var i = 0; i < RemCrypto.NonceBytes; i++)",
     "cs", "two RemSound packets encrypted with the same nonce"),
    ("helper/RemSoundProtocol.cs",
     "return totalParts > 0 && partIndex < totalParts;",
     "return true;",
     "cs", "a PCM part index past its own part count accepted"),
    ("helper/RemSoundProtocol.cs",
     "if (_seen.ContainsKey(nonceId))",
     "if (!_seen.ContainsKey(nonceId))",
     "cs", "a replayed remote-control command accepted"),
    ("helper/RemSoundProtocol.cs",
     "TimeSpan.FromMinutes(10)",
     "TimeSpan.FromMinutes(30)",
     "cs", "a stale remote-control command accepted"),
    ("helper/RemSoundProtocol.cs",
